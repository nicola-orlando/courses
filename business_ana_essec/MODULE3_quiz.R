...
# the sintax of the first arg is like this Rating is the var I want to predict. 
# ~ is a separator between the variable I want to predict and the input features. 
# . denotes I want tall other varaibles to be used as input features. Alternative syntax would be var1+var2+..+varN  
# linreg=lm(Rating~.,data=data) # Estimate a linear regression model of Rating as a function of everything else.
summary(data)
linreg=lm(Rating~Income+Cards+Married,data=data) # Estimate a linear regression model of Rating as a function of everything else.
...
# The logistic regression model has a probability score, doesn't predict the class label. So we have to define a cut value to define if an employee 
# will leave or not the firm
...
sum((logreg$fitted.values<=cutoff))/nrow(datatot) # Compute the percentage of employees who may stay

sum((logreg$fitted.values>cutoff))/nrow(datatot) # Compute the percentage of employees who may leave
